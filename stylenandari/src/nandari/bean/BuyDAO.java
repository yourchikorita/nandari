        ...
        try {
            ...
            for(int i=0; i<lists.size();i++){
                ...
                pstmt = conn.prepareStatement(
                		"select item_count from item where item_id=?");
                pstmt.setInt(1, cart.getItem_id());
                rs = pstmt.executeQuery();
                rs.next();
                
                nowCount = (short)(rs.getShort(1) - 1);
                
                sql = "update item set item_count=? where item_id=?";
                pstmt = conn.prepareStatement(sql);
        ...
        try {
            ...
            while (rs.next()) {
              buy = new BuyBean();
           	  ...
           	  buy.setItem_name(rs.getString("item_name"));
           	  buy.setBuy_price(rs.getInt("buy_price"));
           	  buy.setBuy_count(rs.getByte("buy_count")); 
           	  buy.setItem_image(rs.getString("item_image"));
